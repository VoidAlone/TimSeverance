...
Threads are where you start a process. This is the easiest part of multithreading. All you do is create a new thread from the \<thread\> library and you're set.
...
std::thread th2(some_function, param1, param2, ...);
...
In other words, you have a string, and you can split it into multiple threads, and then join those threads back together to make it into a single string again.
...
So a Mutex is a lockable object that basically wraps a section of code. It says, "Hey, I am about to speak, this section is critical, do not interrupt me." It gets exclusive access to resources when locked. It's basically all the same talking stick rules, but now the talking stick is handcuffed to your hand. People can try to grab the talking stick, but they can't actually get it until it's unlocked and given back. 
...
In c++, it's a common trend that if you new something, you need to destroy it when you're done. When you open something, you've gotta close it. Same deal with locks. If you lock a section, you gotta unlock it. If you start a thread, you have to join it (or detach).
